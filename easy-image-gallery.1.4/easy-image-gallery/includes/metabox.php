...
function Image_Gallery_add_meta_box() {
	...
	$post_types = Image_Gallery_allowed_post_types();
	...
	foreach ( $post_types as $post_type => $status ) {
		add_meta_box( 'Image_Gallery', apply_filters( 'Image_Gallery_meta_box_title', __( 'Image Gallery', 'easy-image-gallery' ) ), 'Image_Gallery_metabox', $post_type, apply_filters( 'Image_Gallery_meta_box_context', 'normal' ), apply_filters( 'Image_Gallery_meta_box_priority', 'low' ) );
	}
...
function Image_Gallery_metabox() {
	...
	$old_meta_structure = get_post_meta( $post->ID, '_Image_Gallery' );
	$new_meta_structure = get_post_meta( $post->ID, '_Image_Gallery_v2' );
	?>
				...
				<div class="eig_repeat_body">
					...
					<div class="eig-template dx-eig-gallery-row row">
						<div class="dx-eig-gallery-row-heading move">
							...
							<span class="eig-remove"><img src="<?php echo Image_Gallery_URL . 'includes/fonts/close.png'; ?>"></span>
							...
							<div class="link-image-to-l">
								<label for="Image_Gallery_link_images_{{row-count-placeholder}}">
									<input type="checkbox" id="Image_Gallery_link_images_{{row-count-placeholder}}" value="on" name="image_gallery[{{row-count-placeholder}}][OPEN_IMAGES]" checked="checked"/> <?php _e( 'Link images to larger sizes', 'easy-image-gallery' ); ?>
								</label>
							</div>
					...
					if ( isset( $new_meta_structure ) && $new_meta_structure != null ) {
						...
						$get_open_images = get_post_meta( $post->ID, '_Image_Gallery_link_images' );
						...
						foreach ( $get_galleries[0] as $gallery ) {
							...
							<div class="dx-eig-gallery-row row">
								<div class="dx-eig-gallery-row-heading move">
									...
									<input type="text" class="dx-eig-shortcode-show" readonly="" value='[Image_Gallery gallery="<?php echo $gallery['SHORTCODE']; ?>"]'>
									<div class="link-image-to-l">
										<label for="Image_Gallery_link_images_<?php echo $gallery_count; ?>">
											<?php
											...
											?>
											<input type="checkbox" id="Image_Gallery_link_images_<?php echo $gallery_count; ?>" value="on" name="image_gallery[<?php echo $gallery_count; ?>][OPEN_IMAGES]"<?php echo $checked; ?> /> <?php _e( 'Link images to larger sizes', 'easy-image-gallery' ); ?>
										</label>
									</div>
									<div class="dx-eig-clear"></div>
								</div>
	...
	<script type="text/javascript">
		...
		jQuery(document).on( 'click', '.dx-eig-insert-shortcode', function(e) {
			...
			var id = $(this).parent().find('.dx-eig-shortcode').val(),
				shortcode = '[Image_Gallery gallery="'+ id +'"]';
...
function Image_Gallery_save_post( $post_id ) {
	...
	if ( isset( $_POST['image_gallery'] ) && ! empty( $_POST['image_gallery'] ) ) {
		...
		update_post_meta( $post_id, '_Image_Gallery_v2', $galleries );
		delete_post_meta( $post_id, '_Image_Gallery' );
	} elseif ( isset( $_POST['action'] ) && 'editpost' == $_POST['action'] ) {
		delete_post_meta( $post_id, '_Image_Gallery_v2' );
	}

	// link to larger images
	if ( isset( $_POST['Image_Gallery_link_images'] ) ) {
		update_post_meta( $post_id, '_Image_Gallery_link_images', $_POST['Image_Gallery_link_images'] );
	} else {
		update_post_meta( $post_id, '_Image_Gallery_link_images', 'off' );
	}
