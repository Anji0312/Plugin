 ...
 * @since 1.0
 */
function Image_Gallery_admin_init() {
	register_setting( 'media', 'easy-image-gallery', 'Image_Gallery_settings_sanitize' );

	// settings
	add_settings_field( 'header', '<h3 class="title">' . __( 'Easy Image Gallery', 'easy-image-gallery' ) . '</h3>', 'Image_Gallery_header_callback', 'media', 'default' );
	add_settings_field( 'lightbox', __( 'Lightbox', 'easy-image-gallery' ), 'Image_Gallery_lightbox_callback', 'media', 'default' );
	add_settings_field( 'post-types', __( 'Post Types', 'easy-image-gallery' ), 'Image_Gallery_post_types_callback', 'media', 'default' );
}
add_action( 'admin_init', 'Image_Gallery_admin_init' );

/**
 * Blank header callback
 * @since 1.0.6
 */
function Image_Gallery_header_callback() {}
 ...
 * @since 1.0
 */
function Image_Gallery_lightbox_callback() {
...
?>
	<select name="easy-image-gallery[lightbox]">
		<?php foreach ( Image_Gallery_lightbox() as $key => $label ) { ?>
...
function Image_Gallery_post_types_callback() {
...
?>
		<?php foreach ( Image_Gallery_get_post_types() as $key => $label ) {
 ...
 * @since 1.0
 */
function Image_Gallery_settings_sanitize( $input ) {

	// Create our array for storing the validated options
	$output = array();

	// lightbox
	$valid = Image_Gallery_lightbox();
	...
	return apply_filters( 'Image_Gallery_settings_sanitize', $output, $input );
 ...
 * @since 1.0
 */
function Image_Gallery_plugin_action_links( $links ) {
