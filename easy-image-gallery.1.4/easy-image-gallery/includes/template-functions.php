...
function Image_Gallery_db_version(){
	$old_meta_structure = get_post_meta(get_the_ID(), '_Image_Gallery', true);
	$new_meta_structure = get_post_meta(get_the_ID(), '_Image_Gallery_v2', true);
 ...
 * Check the POST META
*/
function Image_Gallery_get_post_meta(){
	$db_version = Image_Gallery_db_version();
	if ( $db_version == "new" ) {
		$gallery_ids = get_post_meta(get_the_ID(), '_Image_Gallery_v2', true);
		return $gallery_ids;
	} elseif ( $db_version == "old" ) {
		$get_gallery_attachments = get_post_meta(get_the_ID(), '_Image_Gallery', true);
		$get_gallery_old_data = explode(",", $get_gallery_attachments);

		$get_open_images = get_post_meta(get_the_ID(), '_Image_Gallery_link_images');
 ...
 * @return boolean
 */
function Image_Gallery_is_gallery() {
	$gallery_ids = Image_Gallery_get_post_meta();
 ...
 * @return array
 */
function Image_Gallery_get_image_ids( $post_id = null, $all_galleries_images = true, $gallery_id = null ) {
	global $post;
	...
	if ( $db_version == "new" ) {
		...
		$new_db_structure = get_post_meta(get_the_ID(), '_Image_Gallery_v2', true);
		...
		$old_db_structure = get_post_meta(get_the_ID(), '_Image_Gallery', true);
 ...
 * @return boolean
 */
function Image_Gallery_has_shortcode( $shortcode = '' ) {
	global $post;
 ...
 * @return boolean
 */
function Image_Gallery_has_block( $block = '' ) {
	global $post;
 ...
 * @return array
 */
function Image_Gallery_lightbox() {
	...
	return apply_filters( 'Image_Gallery_lightbox', $lightboxes );
...
if ( !function_exists( 'Image_Gallery_get_lightbox' ) ) :
	function Image_Gallery_get_lightbox() {
...
function Image_Gallery_lightbox_rel( $gallery_id = null ) {
 ...
 * @return array
 */
function Image_Gallery_get_post_types() {
	...
	return apply_filters( 'Image_Gallery_get_post_types', $post_types );
 ...
 * @since 1.0
*/
function Image_Gallery_allowed_post_types() {
 ...
 * @return boolean
 */
function Image_Gallery_allowed_post_type() {
 ...
 * @return string
 */
function Image_Gallery_get_galleries() {
	global $post;
...
function Image_Gallery_shortcode( $atts ) {
	...
	if ( !Image_Gallery_allowed_post_type() ){
        ...
        if ( isset($atts['gallery']) && !empty($atts['gallery']) ){
            return Image_Gallery( $atts['gallery'] );
        }else{
	        return Image_Gallery( 'old_db' );
        }
    }
}
 ...
 * @return integer
 */
function Image_Gallery_count_images( $gallery_shortcode ) {

	$galleries = Image_Gallery_get_post_meta();
 ...
 * @since 1.0
 */
function Image_Gallery( $gallery_id = 'old_db' ) {

	$galleries = Image_Gallery_get_galleries();
    global $post;
    ...
    if ( isset($galleries) && !empty($galleries) ){
        ...
        foreach ($galleries as $gallery){
            ...
            if ( $gallery['SHORTCODE'] == $gallery_id ){
                ...
                // thumbnail count
                $classes[] = $has_gallery_images ? 'thumbnails-' . Image_Gallery_count_images( $gallery['SHORTCODE'], $gallery ) : '';
    			...
    			if ( isset($has_gallery_images) && !empty($has_gallery_images) ) {
                    ...
                    <?php
                    	foreach ( $has_gallery_images as $attachment_id ) {
	                        ...
	                        // get original image
	                        $image_link	= wp_get_attachment_image_src( $attachment_id, apply_filters( 'Image_Gallery_linked_image_size', 'large' ) );
	                        $image_link	= $image_link[0];

	                        $image = wp_get_attachment_image( $attachment_id, apply_filters( 'Image_Gallery_thumbnail_image_size', 'thumbnail' ), '', array( 'alt' => trim( strip_tags( get_post_meta( $attachment_id, '_wp_attachment_image_alt', true ) ) ) ) );
	                        ...
	                        $rel =  Image_Gallery_lightbox_rel( $gallery_id );
	                        ...
	                        echo apply_filters( 'Image_Gallery_html', $html, $rel, $image_link, $image_class, $image_caption, $image, $attachment_id, $post->ID );
	                    }
                	echo '</ul>';

                	if ( Image_Gallery_get_lightbox() === 'luminous' ) {
        ...
        return apply_filters( 'Image_Gallery', $eig_gallery );
    }
}
 ...
 * @since 1.0
 */
function Image_Gallery_append_to_content( $content ) {
	// if it is single page and supported post_type and page not have shortcode.
	if ( is_singular() && is_main_query() && Image_Gallery_allowed_post_type() && !Image_Gallery_has_shortcode('Image_Gallery') && ! Image_Gallery_has_block('wp-block-devrix-easy-image-gallery-block') ) {
		$new_content = Image_Gallery( 'old_db' );
		$content .= $new_content;
	}

	return $content;
}
add_filter( 'the_content', 'Image_Gallery_append_to_content' );
 ...
 * @since 1.0
 */
function Image_Gallery_template_redirect() {
    if ( Image_Gallery_has_shortcode( 'Image_Gallery' ) || Image_Gallery_has_block('wp-block-devrix-easy-image-gallery-block')  )
		remove_filter( 'the_content', 'Image_Gallery_append_to_content' );
}
add_action( 'template_redirect', 'Image_Gallery_template_redirect' );
